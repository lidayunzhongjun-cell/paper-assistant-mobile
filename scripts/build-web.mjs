...
const ocr=path.join(assets,'ocr'),ocrLang=path.join(ocr,'lang');mkdirSync(ocrLang,{recursive:true});
copyFileSync(path.join(root,'node_modules/tesseract.js/dist/worker.min.js'),path.join(ocr,'worker.min.js'));
copyFileSync(path.join(root,'node_modules/tesseract.js/dist/tesseract.min.js'),path.join(ocr,'tesseract.min.js'));
for(const name of ['tesseract-core-lstm.js','tesseract-core-lstm.wasm'])copyFileSync(path.join(root,'node_modules/tesseract.js-core',name),path.join(ocr,name));
copyFileSync(path.join(root,'node_modules/@tesseract.js-data/chi_sim/4.0.0_best_int/chi_sim.traineddata.gz'),path.join(ocrLang,'chi_sim.traineddata.gz'));
copyFileSync(path.join(root,'node_modules/@tesseract.js-data/eng/4.0.0_best_int/eng.traineddata.gz'),path.join(ocrLang,'eng.traineddata.gz'));
...
copyFileSync(path.join(root,'node_modules/tesseract.js/LICENSE.md'),path.join(assets,'TESSERACTJS-LICENSE.txt'));
copyFileSync(path.join(root,'node_modules/tesseract.js-core/LICENSE'),path.join(assets,'TESSERACTJS-CORE-LICENSE.txt'));
