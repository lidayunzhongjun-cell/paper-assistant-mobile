...
import { buildModelGraph, compactConversation, outlineBatches, sanitizeGraphRelations, guideReply, guideExcerpts } from '../web/compact-graph.mjs';
...
test('long summaries, string importance and optional term variations retain content without retries',async()=>{
  ...
  const graph=await buildModelGraph(win,config,paper,signal);
...
test('real missing summaries and overlapping ranges still fail precisely without replacing saved graph',async()=>{
  for(const [revise,error] of [[d=>{d.nodes[0].k=' ';},/L1–L1.*未返回摘要/],[d=>{d.nodes[1].a=1;},/重叠/]]){
    ...
    await assert.rejects(buildModelGraph(win,config,input,signal),error);assert.equal(win.calls.length,2);assert.deepEqual(input.graph,{previous:true});
  }
});
test('long final synthesis remains intact while question context stays bounded',async()=>{
  ...
  const win=revisedFixture(d=>{if(!d.nodes)d.brief=final;});const graph=await buildModelGraph(win,config,input,signal);
...
test('imported summary guides graph while original offsets and source evidence remain intact',async()=>{
  ...
  const graph=await buildModelGraph(win,config,input,signal);
  ...
  await assert.rejects(buildModelGraph(win,config,{...paper,graphBuildMode:'summary'},signal),/重新导入/);
});
...
test('invalid optional relationships do not discard a complete source-grounded outline',async()=>{
  const win=fixtureModel(false,true),graph=await buildModelGraph(win,config,paper,signal);
...
test('guide compression accepts prose, nested JSON, and outputs beyond the old 2600 character gate',async()=>{
  ...
  for(const wrap of [text=>text,text=>JSON.stringify({brief:text}),text=>JSON.stringify({outline:[text]})]){
    ...
    const graph=await buildModelGraph(win,config,{...paper,graphBuildMode:'summary',graphSummary:{name:'guide.docx',text:'原始总结。'.repeat(6305)}},signal);
...
test('small paper gets complete source offsets, terms and core in one model call',async()=>{
  const win=fixtureModel();const graph=await buildModelGraph(win,config,paper,signal);
...
test('incomplete batch retries once without committing a partial graph',async()=>{
  const win=fixtureModel(true);const graph=await buildModelGraph(win,config,paper,signal);assert.equal(win.calls.length,2);assert.ok(graphUsable({...paper,graph}));
});
test('long paper uses one source pass plus compact synthesis and bounded input',async()=>{
  ...
  const win=fixtureModel();const graph=await buildModelGraph(win,config,long,signal);
...
test('raw details missing from the outline and chapter relationships remain retrievable',async()=>{
  ...
  const graph=await buildModelGraph(fixtureModel(),config,target,signal);target.graph=graph;
